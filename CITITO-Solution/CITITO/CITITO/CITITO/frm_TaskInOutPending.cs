...
namespace CITITO
{
    public partial class frm_TaskInOutPending : MetroForm
    {
        SqlConnection conn;

        //Start Pass insatance when form is already opend or not
        private static frm_TaskInOutPending _instance;
        public static frm_TaskInOutPending GetInstance(string uPIC, string uMUID)
        {
            ...
            if (_instance == null || _instance.IsDisposed)
            {
                ...
                _instance = new frm_TaskInOutPending(mPIC, mMUID);
        ...
        public frm_TaskInOutPending(string uPIC, string uMUID)
        {
            InitializeComponent();
        ...
        private void RefreshData()
        {
            ...
            if (new PICHeaderMng(this.conn).PICIsExist(uPIC))
            {
                ...
                //Fill Datagridview
                dataGridViewTaskInOutForApproval.DataSource = new TaskRecordDetailMng(this.conn).GetPendingTaskInOutRecordByPICStatusPending(uPIC, uFrom, uTo);
            ...
            if (!new PICHeaderMng(this.conn).PICIsExist(uPIC) && new ManagerHeaderMng(this.conn).ManagerIsExist(uMID))
            {
                ...
                dataGridViewTaskInOutForApproval.DataSource = new TaskRecordDetailMng(this.conn).GetPendingTaskInOutRecordByManagerStatusPending(uMID, uFrom, uTo);
            }
        ...
        private void frm_TaskInOutPending_Load(object sender, EventArgs e)
        {
            RefreshData();
            ...
            dataGridViewTaskInOutForApproval.Columns[0].Width = 40;
            dataGridViewTaskInOutForApproval.Columns[4].Width = 105;
            dataGridViewTaskInOutForApproval.Columns[5].Width = 55;
            dataGridViewTaskInOutForApproval.Columns[11].Width = 50;
        }
        ...
        private void cmbMUID1_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            dataGridViewTaskInOutForApproval.DataSource = new TaskRecordDetailMng(this.conn).GetPendingTaskInOutRecordByManagerStatusPending(uMUID, uFrom, uTo);
        }
        ...
        private void cmbUserID_KeyUp(object sender, KeyEventArgs e)
        {
            ...
            if (cmbMUID1.Enabled)
            {
                ...
                {
                    dataGridViewTaskInOutForApproval.DataSource = new TaskRecordDetailMng(this.conn).GetPendingTaskInOutRecordByPICAndUIDStatusPending(uUID, uPIC);
                }
                else
                {
                    RefreshData();
            ...
            else
            {
                if (!String.IsNullOrEmpty(uUID))
                {
                    dataGridViewTaskInOutForApproval.DataSource = new TaskRecordDetailMng(this.conn).GetPendingTaskInOutRecordByManagerAndUIDStatusPending(uUID, uMUID);
                }
                else
                {
                    RefreshData();
        ...
        private void cmbPCPCode_KeyUp(object sender, KeyEventArgs e)
        {
            ...
            if (cmbMUID1.Enabled)
            {
                ...
                {
                    dataGridViewTaskInOutForApproval.DataSource = new TaskRecordDetailMng(this.conn).GetPendingTaskInOutRecordByPICAndPCPCodeStatusPending(uPCPCode, uPIC);
                }
                else
                {
                    RefreshData();
            ...
            else
            {
                if (!String.IsNullOrEmpty(uPCPCode))
                {
                    dataGridViewTaskInOutForApproval.DataSource = new TaskRecordDetailMng(this.conn).GetPendingTaskInOutRecordByManagerAndPCPCodeStatusPending(uPCPCode, uMUID);
                }
                else
                {
                    RefreshData();
        ...
        private void cmbFileName_KeyUp(object sender, KeyEventArgs e)
        {
            ...
            if (cmbMUID1.Enabled)
            {
                ...
                {
                    dataGridViewTaskInOutForApproval.DataSource = new TaskRecordDetailMng(this.conn).GetPendingTaskInOutRecordByPICAndFileNameStatusPending(uFileName, uPIC);
                }
                else
                {
                    RefreshData();
            ...
            else
            {
                if (!String.IsNullOrEmpty(uFileName))
                {
                    dataGridViewTaskInOutForApproval.DataSource = new TaskRecordDetailMng(this.conn).GetPendingTaskInOutRecordByManagerAndFileNameStatusPending(uFileName, uMUID);
                }
                else
                {
                    RefreshData();
        ...
        private void btnCheck_Click(object sender, EventArgs e)
        {
            ...
            if (cmbMUID1.Enabled)
            {
                dataGridViewTaskInOutForApproval.DataSource = new TaskRecordDetailMng(this.conn).GetPCPRecordByDateRangeByPICStatusPending(uPIC, uFrom, uTo);
            }
            else
            {
                dataGridViewTaskInOutForApproval.DataSource = new TaskRecordDetailMng(this.conn).GetPCPRecordByDateRangeStatusPending(uMUID, uFrom, uTo);
            }
        }
                    ...
                    try
                    {
                        ...
                        if (!System.IO.File.Exists(fileName))
                        {
                            using (System.IO.FileStream fs = System.IO.File.Create(fileName))
                            {
                                using (XLWorkbook wb = new XLWorkbook())
                                {
                                    ...
                                    var ws = wb.Worksheets.Add(dt, "CITITO_" + lblMUID.Text + "_Pending Task Records");
                                    //ws.Range("A1:A1048576").Delete(XLShiftDeletedCells.ShiftCellsLeft);
                                    wb.SaveAs(fs);

                                    MetroFramework.MetroMessageBox.Show(MetroForm.ActiveForm, "\nTask pending records successfully export to \"" + fileName + "\".", "Records Export!", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        ...
                        else
                        {
                            using (XLWorkbook wb = new XLWorkbook())
                            {
                                ...
                                MetroFramework.MetroMessageBox.Show(MetroForm.ActiveForm, "\nTask pending records successfully export to \"" + fileName + "\" path.", "Records Export!", MessageBoxButtons.OK, MessageBoxIcon.Information);
