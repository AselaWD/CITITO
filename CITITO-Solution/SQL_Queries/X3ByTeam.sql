CREATE PROC DBoard_X3byTask_User @mPIC nvarchar(20), @mFrom DateTime, @mTo DateTime
...
X3 AS (
	SELECT TOP (999999999999999999) X.TR_UID, X.TR_MID, X.PC_ProcessCode, X.Task_ID, X.WORKDATE, [OUTPUT], X1, [WORK_HOURS], (X.X1/ISNULL(Y1.Y,1))*100 AS [X3]
	FROM Y Y1
	LEFT JOIN X1 X ON X.TR_UID =Y1.TR_UID AND X.WORKDATE = Y1.WORKDATE AND X.TR_MID= Y1.TR_MID
)

SELECT X.WORKDATE, X.TR_UID AS [UID], X.TR_MID AS [TEAM], td.PIC_Project AS [PROJECT], X.PC_ProcessCode AS [ITEM_CODE], X.Task_ID AS [TASK_CODE], td.Task_Description AS [DESC.],  ROUND(SUM(X.[OUTPUT]),2) AS [OUTPUT], ROUND(SUM(X.[WORK_HOURS]),2) AS [WORK_HOURS], ROUND(SUM(X.X1),2) AS [X1], ROUND(SUM(X.X3),2) AS [X3] 
